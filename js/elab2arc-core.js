...
var blobb = [];
    ...
    const updateelabList = async (search) => {
    ...
    const elabPreview = async (element) => {
      loading.show();
      ...
      await loadExperiment(instance, elabid, elabtoken, element.dataset.type);
    ...
    const updateARCList = async (search) => {
      const { elabtoken, datahubtoken, instance, elabidList } = await getParameters();
      // Fetch user info
      const id = await fetchUser(datahubtoken);
    ...
    const fillElabTable = async (query = "?order=id&sort=des&scope=1&limit=99", tableid = "elabTable", action = "update", typeConfig = window.typeConfig, targetId) => {
      try {
        // Define the API endpoint for fetching user-related projects
