...
var conversionStartTime = null; // Track when conversion starts
    ...
    // (primary: zap/[redacted-ip], backup: luxvps2/[redacted-ip]) so the
...
`;
    ...
    /**
     * Short, protocol-free host label for user-facing messages (e.g.
     * "git.nfdi4plants.org" or "github.com"), so push/status messages read
     * correctly for any configured DataHub/GitLab/GitHub host instead of a
     * hardcoded "PLANTDataHUB" that was only ever true for the default host.
     * @param {string} [url] - repo or base URL; defaults to the configured DataHub URL
     * @returns {string}
     */
    function getShortHostLabel(url) {
      const target = url || getDatahubURL();
      try {
        return new URL(target).host;
      } catch (e) {
        return String(target || '').replace(/^https?:\/\//, '').replace(/\/.*$/, '');
      }
    }
...
Date: ${timestamp}`;
      ...
      try {
        ...
        updateInfo(`Pushing to ${getShortHostLabel(datahubURL)} (main branch)...`, pushProgressStart);
        ...
        updateInfo(`${getShortHostLabel(datahubURL)} has been updated.  <br>`, pushProgressEnd, [{ label: '🔗 View ARC', url: pushLinkUrl }]);
        reset401RetryCounter('datahub');
        //
      } catch (error) {
        ...
        if (!pushProxy && error.message && (error.message.includes('fetch') || error.message.includes('network') || error.message.includes('CORS'))) {
          ...
          try {
            ...
            updateInfo(`${getShortHostLabel(datahubURL)} has been updated (via proxy).  <br>`, pushProgressEnd, [{ label: '🔗 View ARC', url: proxyLinkUrl }]);
            return;
          } catch (proxyError) {
        ...
        updateInfo(`Pushing to ${getShortHostLabel(datahubURL)} (master branch)...`, pushProgressStart);
        ...
        try {
          ...
          updateInfo(`${getShortHostLabel(datahubURL)} has been updated (master branch).  <br>`, pushProgressEnd, [{ label: '🔗 View ARC', url: masterLinkUrl }]);
        } catch (masterError) {
...
Generated by elab2ARC`
        ...
        setTimeout(() => {
          showSuccessToast(`Success! All ${totalEntries} eLabFTW entries have been converted to ARC format and pushed to ${getShortHostLabel()}.`, 10000);
        }, 500);
      ...
      try {
        ...
        if (summary.writtenPaths.length > 0) {
          ...
          if (gitlabURL && datahubtoken && gitlabURL !== 'GitLab URL') {
            ...
            showWarningToast('READMEs generated but not pushed: ARC URL or token missing.');
          }
        }
      } catch (error) {
    ...
    window.commitPushReadmesFromModal = async function() {
      try {
        ...
        const datahubtoken = document.getElementById('datahubToken').value;
        if (!gitlabURL || !datahubtoken || gitlabURL === 'GitLab URL') {
          showWarningToast('Please ensure ARC URL and token are set before pushing.');
          return;
        }
...
`;
    ...
    function handleError(error) {
      if (error.message.includes("401")) {
        ...
        showError(`Error: ${getShortHostLabel()} cannot be accessed. Please verify that the datahub_url in the extra_fields of eLabFTW is correct.`);
      } else {
      ...
      try {
        ...
        if (!fs.existsSync(`./${gitRoot}`)) {
          ...
          if (!gitlabURL || gitlabURL.includes("Please select")) {
            showWarningToast("ARC not found locally and no ARC URL available. Please select the ARC again.");
            return;
          }
